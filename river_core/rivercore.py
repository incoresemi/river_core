...
def rivercore_merge(config_file, verbosity, db_files, output_db):
    ...
    for target in target_list:
        ...
        try:
            ...
            header_str = '<h2>Plugin Coverage Reports</h2>\n'
            # TODO Vinay check naming
            ranked_report_str = '<h3><a href={0}.html>Ranked Coverage Merge Report on {1}</h3></a>\n'.format(
                './' + output_db + '_html/index', merge_time)

            html_file = open(report_file[0], 'r+')
            data = html_file.readlines()
            # Check if merge report exists
            for data_index in range(-3, 0):
                if 'Coverage Merge Report on ' in data[data_index]:
                    data[data_index] = report_str
                    break
                else:
                    data.insert(-1, header_str + report_str)
                    break

            for rdata_index in range(-2, 0):
                if 'Ranked Coverage Merge Report' in data[rdata_index]:
                    data[rdata_index] = ranked_report_str
                    break
                else:
                    data.insert(-1, ranked_report_str)
                    break
            ...
            logger.info(
                'Something went wrong trying to modifying the existing report\nExiting the framework'
            )
