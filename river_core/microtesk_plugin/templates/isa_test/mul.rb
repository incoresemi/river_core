...
class RandomTemplate < RiscVBaseTemplate
  ...
  def run
    ...
    sequence {
      mul(x(_ FREE), x(_ FREE), (_ do testdata('boundary') end)) do situation('random_biased',
    ...
    if is_rev('RV64I') then
      sequence {
        mulw(x(_ FREE), x(_ FREE), (_ do testdata('boundary') end)) do situation('random_biased',
