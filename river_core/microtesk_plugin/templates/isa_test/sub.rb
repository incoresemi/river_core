...
class RandomTemplate < RiscVBaseTemplate
  ...
  def run
    ...
    sequence {
      sub(x(_ FREE), x(_ FREE), (_ do testdata('boundary') end)) do situation('random_biased',
    ...
    if is_rev('RV64I') then
      sequence {
        subw(x(_ FREE), x(_ FREE), (_ do testdata('boundary') end)) do situation('random_biased',
