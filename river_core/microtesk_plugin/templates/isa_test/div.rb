...
class RandomTemplate < RiscVBaseTemplate
  ...
  def run
    ...
    sequence {
      div(x(_ FREE), x(_ FREE), (_ do testdata('boundary') end)) do situation('random_biased',
    ...
    if is_rev('RV64I') then
      sequence {
        divw(x(_ FREE), x(_ FREE), (_ do testdata('boundary') end)) do situation('random_biased',
