...
class RandomTemplate < RiscVBaseTemplate
  ...
  def run
    ...
    sequence {
      rem(x(_ FREE), x(_ FREE), (_ do testdata('boundary') end)) do situation('random_biased',
    ...
    if is_rev('RV64I') then
      sequence {
        remw(x(_ FREE), x(_ FREE), (_ do testdata('boundary') end)) do situation('random_biased',
