...
def gen_cmd_list(gen_config):
    ...
    #args = ''
    run_command = []
    logger.debug('Building the generator')
    sys_command('rm -rf workdir')
    sys_command('run --co -o workdir')
    for test_item in gen_list:
        logger.debug('run --test {0} --steps gen'.format(test_item['test']))
        run_command.append('run --test {0} --steps gen --noclean -o /scratch/lavanya/generator/river_core/workdir'.format(test_item['test']))
    return run_command
    ...
    #            test_prefix = 'riscv_dv_{0}_{1}_{2}'.format(template_name.replace('.rb', ''), gen_prefix, i)
...
def idfnc(val):
  template_match = re.search('--test (.*) --steps ', '{0}'.format(val))
...
def test_input(request, autouse=True):
    ...
    template_match = re.search('--test (.*) --steps ', program)
