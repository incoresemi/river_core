...
class CompilePlugin(object):
    ...
    def compile(self, regress_list, command_line_args, jobs, filter):
        ...
        pwd = os.getcwd()
        pytest_file = pwd + '/river_core/cclass_plugin/gen_framework.py'
        print(pytest_file)

        #pytest.main([pytest_file, '-n={0}'.format(jobs), '--addopts=--filter={0}'.format(filter), '-v', '--html=microtesk_compile.html', '--self-contained-html'])
        pytest.main([pytest_file, '--name=FFFVIVEK', '-n={0}'.format(jobs), '-v', '--html=microtesk_compile.html', '--self-contained-html'])
