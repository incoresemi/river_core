...
class RiscvDvPlugin(object):
    ...
    def gen(self, gen_config, jobs, filter, norun):
        ...
        pwd = os.getcwd()
        pytest_file = pwd + '/river_core/riscv_dv_plugin/gen_framework.py'
        print(pytest_file)
        #pytest.main([pytest_file, '-n={0}'.format(jobs), '-v', '--html=microtesk_gen.html', '--self-contained-html'])

        if norun:
            # to display test items
            pytest.main([pytest_file, '--collect-only', '-n={0}'.format(jobs), '-k={0}'.format(filter), '--configlist={0}'.format(gen_config), '-v', '--html=riscv_dv_gen.html', '--self-contained-html'])
        else:
            pytest.main([pytest_file, '-n={0}'.format(jobs), '-k={0}'.format(filter), '--configlist={0}'.format(gen_config), '-v', '--html=riscv_dv_gen.html', '--self-contained-html'])
